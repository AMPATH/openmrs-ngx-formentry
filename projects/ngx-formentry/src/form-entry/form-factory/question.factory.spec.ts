...
describe('Question Factory', () => {
  ...
  const dateSchemaQuestion: any = {
    ...
    validators: [
      ...
      },
      {
        type: 'js_expression',
        failsWhenExpression:
          '(new moment(encDate)).isBefore((new moment(myValue)), "day")' +
    ...
    questions: [
      {
        ...
        hide: {
          hideWhenExpression:
            'arvPlan !== "a899e0ac-1350 - 11df-a1f1 - 0026b9348838"'
        }
      },
      {
        ...
        hide: {
          hideWhenExpression:
            'artEligibleNotStarted !== "a8aaf3e2-1350 - 11df-a1f1 - 0026b9348838"'
        }
      }
    ]
  };
        ...
        sections: [
          {
            ...
            questions: [
              ...
              },
              {
                ...
                hide: {
                  hideWhenExpression:
                    'onArt !== "a899b35c-1350 - 11df-a1f1 - 0026b9348838"'
                }
              },
              {
  ...
  it('should convert schema select question to select question model', () => {
    ...
    converted.showHistoricalEncounterDate(
    ...
    expect(converted.options).toEqual(
      selectSchemaQuestion.questionOptions.answers.map(function (obj) {
        return {
          label: obj.label,
          value: obj.concept
        };
    ...
    expect(converted.renderingType).toEqual(
      selectSchemaQuestion.questionOptions.rendering
    );
  ...
  it('should convert schema multiCheckbox question to MultiCheckbox question model', () => {
    const converted = factory.toMultiCheckboxQuestion(
      multiCheckboxSchemaQuestion
    );
    ...
    expect(converted.options).toEqual(
      multiCheckboxSchemaQuestion.questionOptions.answers.map(function (obj) {
        return {
          label: obj.label,
          value: obj.concept
        };
  ...
  it('should convert schema Person Attribute question to Person Attribute question model', () => {
    const converted = factory.toPersonAttributeQuestion(
      personAttributeSchemaQuestion
    );
  ...
  it('should convert schema encounter Provider question to Encounter Provider question model', () => {
    const converted = factory.toEncounterProviderQuestion(
      encounterProviderSchemaQuestion
    );
  ...
  it('should convert select-concept-answers to remote select question', () => {
    const conceptAnswerSchema = {
      ...
      required: {
        ...
        message:
          'Patient marked as receiving new WHO stage. Please select stage.',
    ...
    const converted: SelectQuestion = factory.toConceptAnswerSelect(
      conceptAnswerSchema
    );
  ...
  it('should convert an endpoint remote-select schema question to a remote-select question carrying the endpoint config', () => {
    ...
    const converted = factory.toRemoteSelectQuestion(endpointSchema);

    expect(converted).toBeDefined();
    expect(converted.label).toEqual('Doctor');
    expect(converted.key).toEqual('doctor');
    expect(converted.renderingType).toEqual('remote-select');
    expect(converted.extras).toEqual(endpointSchema);
  ...
  it('should carry the endpoint config through unchanged, leaving key defaults to the data source', () => {
    const endpointSchema: any = {
      label: 'Provider',
  ...
  it('should convert form schema to a list of question models, without pages', () => {
    ...
    expect(convertedSample2.questions[0].label).toEqual(
      'Person bringing patient:'
    );
    expect(
      ((convertedSample2.questions[0] as SelectQuestion).options[2] as any)
        .label
    ).toEqual('Father');
  ...
  it('should convert complex obs schema question to field set question model', () => {
    ...
    expect((result.questions[0] as QuestionBase).label).toBe(
      'Creatinine mmol/L:'
    );
    expect((result.questions[0] as QuestionBase).key).toBe('creat_test');
    expect((result.questions[0] as QuestionBase).extras).toBe(
      schemaQuestion.questions[0]
    );
    expect((result.questions[0] as QuestionBase).renderingType).toBe('number');

    expect((result.questions[1] as QuestionBase).label).toBe(
      'Date of Creatinine mmol/L:'
    );
    expect((result.questions[1] as QuestionBase).key).toBe('date_creat_test');
    expect((result.questions[1] as QuestionBase).extras).toBe(
      schemaQuestion.questions[1]
    );
  ...
  it('should convert old schema version complex obs schema question to field set question model', () => {
        ...
        shownDateOptions: {
          validators: [
            ...
            },
            {
              type: 'js_expression',
              failsWhenExpression:
                '!isEmpty(creatinine_test) && isEmpty(myValue)',
              message: 'Date result is required.'
            }
          ],
          hide: {
    ...
    expect((result.questions[0] as QuestionBase).extras).toEqual(
      expectedConvertedSchema.questions[0]
    );
    expect((result.questions[0] as QuestionBase).renderingType).toBe('number');
    ...
    expect((result.questions[1] as QuestionBase).key).toBe(
      'date_creatinine_test'
    );
    expect((result.questions[1] as QuestionBase).extras).toEqual(
      expectedConvertedSchema.questions[1]
    );
    expect((result.questions[1] as QuestionBase).renderingType).toBe('date');
  });
});
